...
def compute_log_power_spectrum_2d(img: np.ndarray) -> np.ndarray:
    """Compute the 2-D log10 power spectrum of a grayscale image.

    Applies a 2-D FFT, shifts the zero-frequency component to the center,
    and returns log10(|F(u, v)|² + ε).  Taking the logarithm *before*
    azimuthal averaging matches the Keuper et al. (CVPR 2020) methodology:
    each pixel contributes its log-power to the radial bin average rather
    than its linear power.
    ...
    -------
    np.ndarray
        2-D log10 power spectrum of shape (H, W), dtype float64.
    """
    f = np.fft.fft2(img)
    f_shifted = np.fft.fftshift(f)
    log_power = np.log10(np.abs(f_shifted) ** 2 + _EPSILON)
    return log_power.astype(np.float64)


def batch_log_power_spectra(images: list[np.ndarray]) -> np.ndarray:
    """Compute 2-D log10 power spectra for a list of images.
    ...
    -------
    np.ndarray
        Stacked log10 power spectra of shape (N, H, W), dtype float64.
    ...
    spectra = [compute_log_power_spectrum_2d(img) for img in images]
